...
public class Main extends JPanel{
    ...
    /*
     * --- TRANSPORT AND DRONE ARE SYNONYMOUS ---
     */
    ...
    private ArrayList<Astronaut> nauts = new ArrayList<Astronaut>();
    //THE FOLLOWING ARE USED FOR:
    /*  1.DETERMINING WHEN AN INVADER/ASTEROID/PARTICLE/MAGNETIC CLOUD SHOULD BE ADDED
     *  2.DRAWING THE INVADER/ASTEROID/PARTICLE/MAGNETIC CLOUD IF IT EXISTS
     */
    ...
    private ArrayList<MagneticCloud> clouds = new ArrayList<MagneticCloud>();
    private boolean magnetic;
    private MagneticCloud mc;
    ...
    public void paint(Graphics g){
        ...
        if(invaders.size()!=0){
            invaders.get(0).paint(g); //AT ANY INSTANT THERE WILL EITHER BE 0 OR 1 ELEMENTS IN THE INVADER/ASTEROID/PARTICLE/MAGNETIC CLOUD ARRAYLISTS SO arraylist.get(0) CAN SAFELY BE CALLED AFTER CHECKING IF THE SIZE ISN'T 0
        ...
        //paints magnetic cloud if magnetic cloud exists
        if(magnetic){
            mc.paint(g);
            mc.move();
        }
        //determines if magnetic cloud should continue to be painted
        if(clouds.size()!=0){
            clouds.get(0).paint(g);
            clouds.get(0).move();
            if(!clouds.get(0).inBounds()){
                clouds.remove(0);
            }
        }
    ...
    public ArrayList<MagneticCloud> getCloudList(){
        return clouds;
    }
    
    public void addMagneticCloud(MagneticCloud mc){
        clouds.add(mc);
    }
    ...
    public void addAstronaut(){
        nauts.add(new Astronaut());
    }
    
    public void powerSurge(int destroy){ //THIS METHOD IS FOR THE MAGNETIC STORM, IT TAKES IN THE AMOUNT OF DRONES THAT WILL BE DESTROYED BY THE POWER SURGE CAUSED BY THE MAGNETIC STORM
        int counter = 0; //TO DETERMINE WHEN THE AMOUNT OF DESTROYED DRONES IS EQUAL TO THE AMOUNT THAT SHOULD BE DESTROYED
        for(int i = 0; i < transmods.size() && counter < destroy; i++){
            transmods.remove(i);
            counter++;
        }
    }
